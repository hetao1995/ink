...
public class CommonValidator {
    ...
    public static void valid(ContentVo contentVo) {
        if (stringLengthNotBetween(contentVo.getTitle(), WebConstant.MIN_CONTENT_TITLE_LENGTH, WebConstant.MAX_CONTENT_TITLE_LENGTH)) {
            throw new TipException(ExceptionEnum.CONTENT_TITLE_ILLEGAL);
        }
        if (stringLengthNotBetween(contentVo.getContent(), WebConstant.MIN_CONTENT_TEXT_LENGTH, WebConstant.MAX_CONTENT_TEXT_LENGTH)) {
    ...
    public static void valid(CommentVo commentVo) {
        if (stringLengthNotBetween(commentVo.getContent(), WebConstant.MIN_COMMENT_TEXT_LENGTH, WebConstant.MAX_COMMENT_TEXT_LENGTH)) {
    ...
    private static boolean stringLengthNotBetween(String str, int start, int end) {
        return StringUtils.isBlank(str) || str.length() < start || str.length() >= end;
    }
}
